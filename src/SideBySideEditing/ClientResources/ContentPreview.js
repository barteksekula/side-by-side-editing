define([
    ...
    "side-by-side-editing/AutoResizingIframe", //Used in template
    "epi/shell/widget/overlay/OverlayContainer",
    "epi/shell/layout/_LayoutWidget",

    // Resources
    "dojo/text!./templates/ContentPreview.html",
], function (
    // Dojo
    declare,
    lang,
    ...
    return declare([_LayoutWidget, _TemplatedMixin, _WidgetsInTemplateMixin, _ScrollbarMeasurementMixin], {
        ...
        startup: function () {
            ...
            this.own(
                aspect.after(this.iframe, "onElementResize", function (properties) {
                    this.overlay.updateOverlayItemBoundaries(properties);
                }.bind(this), true)
            );

            this.own(
                aspect.after(this.iframe, "onUnload", function (url) {
            ...
            this.iframe.watch("siteProperties", function (name, oldValue, value) {
                if (this.sent) {
                    return;
                }
                oldValue && this.overlay.updateOverlayItemBoundaries(value);
                // publish event that properties have been created
                this.sent = true;
                (!oldValue || (oldValue.length !== value.length)) && topic.publish("/propertiesCreated");
            }.bind(this));
        },
        ...
        isOverlayPrerequisitesReady: function () {
            ...
            var props = this.iframe.siteProperties;
            return (doc || props);
        },
