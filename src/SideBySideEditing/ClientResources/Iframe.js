define([
    ...
    return declare([_Widget, _TemplatedMixin, DestroyableByKey], {
        ...
        buildRendering: function () {
            ...
            this.own(
                postMessage.subscribe(this.name + "/site/mouseup", function () {
                    ...
                    // publish event that a click happened in the original site, useful to close up UI elements living in a different domain
                    topic.publish("/site/click");
                }.bind(this)),
                postMessage.subscribe(this.name + "/site/properties", function (message) {
                    if (message && message.properties) {
                        this.set("siteProperties", message.properties.map(function (node) {
        ...
        isInspectable: function (/*bool */clearCache) {
            ...
            //    accessible by code i.e. in the same domain or if properties are posted from iframe to site properties manually
        ...
        load: function (url, parameters, mixWithExistingParams, forceReload) {
            ...
            if (forceReload || ApplicationSettings.limitUI || !(newUrl.path === currentUrl.path && epi.areEqual(newUrl.query, currentUrl.query))) {
                doLoad(newUrl, true);
